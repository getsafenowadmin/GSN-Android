...
class VectorSettingsHelpAboutFragment :
    ...
    override fun bindPref() {
        ...
        // Copyright
        findPreference<VectorPreference>(VectorPreferences.SETTINGS_ACCEPTABLE_USE_POLICY_PREFERENCE_KEY)!!
                .onPreferenceClickListener = Preference.OnPreferenceClickListener {
            if (firstThrottler.canHandle() is FirstThrottler.CanHandlerResult.Yes) {
                openUrlInChromeCustomTab(requireContext(), null, VectorSettingsUrls.ACCEPTABLE_USE_POLICY)
            }
            false
        }
